...
def find_gpu_id(input_id):
    for i in range(8):  # 假设有8个可能的节点
        node_id = cpu_nodes + i
        node_properties_path = f"{sys_layout_dir}/nodes/{node_id}/properties"
        try:
            with open(node_properties_path, "r") as file:
                for line in file:
                    if "mgpu_id" in line:
                        parts = line.split()
                        if len(parts) > 1 and int(parts[1]) == input_id:
                            return i  # 如果找到匹配的socket_id，返回对应的索引i
        except FileNotFoundError:
            ...
            print(f"An error occurred: {e}")
            continue
    return None  # 如果没有找到匹配的socket_id，返回None


def get_gpu_id_map():
    gpu_ip_map :map[int][int]= None
    for i in range(8):  # 假设有8个可能的节点
        node_id = cpu_nodes + i
        node_properties_path = f"{sys_layout_dir}/nodes/{node_id}/properties"
        try:
            ...
            print(f"An error occurred: {e}")
            continue
    return gpu_ip_map  # 如果没有找到匹配的socket_id，返回None
...
def main():
    ...
    # 添加 --pairs 参数来接收数组对
    parser.add_argument('--pairs', metavar='PAIR', type=parse_int_pair, nargs='+',
    ...
    exec_path = os.getenv("SWITCHBOX_CHECK_TOOL_PATH", "/opt/mxmap/bin/client")
    ...
    for pair in args.pairs:
        (gpuA, gpuB) = pair
        print(f"GPU Pair for Box{index}: {gpuA}, {gpuB}")

        gpuAID = find_gpu_id(gpuA)
        gpuBID = find_gpu_id(gpuB)

...
std::unordered_map<uint64_t, uint64_t> MacaGpuTimer::nccl_seq_num{};
std::unordered_map<std::string, int> MacaGpuTimer::tracing_metas_{};
std::unordered_map<int, uint64_t> MacaGpuTimer::trace_id_counter_{};
std::unordered_map<cudaStream_t, EventStartTimeHelper*>
    MacaGpuTimer::stream_timer_helper_{};
int MacaGpuTimer::kernel_encoding_counter_(0);
 ...
 * ===================================
 */
const std::string_view& MacaGpuTimer::getType() { return type_; }

const uint64_t MacaGpuTimer::getProblemSize() { return problem_size_; }

// void MacaGpuTimer::startRecord() { cudaEventRecord(start_event_, stream_); }

void MacaGpuTimer::startRecord() {;}

void MacaGpuTimer::endRecord() {
  if (is_host_) {
...
int MacaGpuTimer::getTraceCode() { return trace_code_; }

uint64_t MacaGpuTimer::getTraceId() { return trace_id_; }

const std::string MacaGpuTimer::getName() { return name_; }

bool MacaGpuTimer::isReady() {
...
time_t MacaGpuTimer::getExecuteTimeStamp() {
  auto execute_time = stream_timer_helper_[stream_]->getTime(
      start_event_, &(this->is_validate_to_trace_));
  return execute_time;
}

time_t MacaGpuTimer::getLaunchTimeStamp() { return launch_time_timestamp_; }

uint64_t MacaGpuTimer::getDuration() {
  // if (is_host_) {
...
void MacaGpuTimer::reBuild() { name_ = rebuild_cb_(); }

Labels MacaGpuTimer::getExtraLabels() { return extra_labels_; }

bool MacaGpuTimer::isHang(time_t timeout) {
...
bool MacaGpuTimer::ignoreHang() { return is_barrier_; }

bool MacaGpuTimer::isHost() { return is_host_; }
...
void MacaGpuTimer::doPrepareForDumpTrace() {
  for (auto it : stream_timer_helper_) it.second->reset();
}

void MacaGpuTimer::doPrepare() {
...
void MacaGpuTimer::dumpTraceMeta(const std::string& path,
...
std::string MacaGpuTimer::collBucketFn(double performance,
...
std::string MacaGpuTimer::matmulBucketFn(double peformance,
...
void MacaGpuTimer::reset_cb(
    std::function<MacaGpuTimer::FnReturn(MacaGpuTimer*)> cb) {
...
void MacaGpuTimer::reset(
    cudaStream_t s, std::function<MacaGpuTimer::FnReturn(MacaGpuTimer*)> cb,
...
void MacaGpuTimer::reset_stream(cudaStream_t s,
                                  const std::string_view& type) {
  is_host_ = false;
  stream_ = s;
...
void MacaGpuTimer::reset(cudaStream_t s, const std::string_view& type,
...
bool MacaGpuTimer::isValidateToTrace() { return is_validate_to_trace_; }

/*
 * ===================================
 * Class impl of MacaGpuTimer::MatmulBuilderCallback
 * ===================================
 */

MacaGpuTimer::FnReturn MacaGpuTimer::MatmulBuilderCallback::operator()(
    MacaGpuTimer* timer) {
  std::ostringstream oss;
...
void MacaGpuTimer::MatmulBuilderCallback::reset(
    const std::initializer_list<int64_t>& bmnk,
    const std::initializer_list<int64_t>& ld,
...
void NcclCommWrapper::registerFunction() {
  static std::string nccl_version = "2.16.5";

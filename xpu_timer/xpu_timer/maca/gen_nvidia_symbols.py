...
dtype_mapping = {**flash_attn_dtype_mapping, **mccl_dtype_mapping}
...
def list_loaded_libraries_linux():
    ...
    for line in lines:
        ...
        elif "libmccl.so" in line:
            nccl_lib = line.strip().split(" ")[-1]
    if nccl_lib is None and has_torch:
...
def nccl_prser(nccl_lib):
    def parse_2_16_5(nccl_lib):
        pattern = re.compile(
            r"([0-9a-f]+) \w __device_stub__mcclKernel_(Broadcast|AllReduce|ReduceScatter|AllGather|SendRecv|Reduce)_([A-Z0-9_]+)_(Sum|Prod|Max|Min)_([a-z0-9_]+)\(mcclDevComm\*, unsigned long, mcclWork\*\)"
        )
...
def search_flash_attn(flash_attn_lib, addr_to_name):
    ...
    flash_attn_dtype = re.compile(r"mctlass::([a-zA-Z0-9_]+)")
...
def search_nccl(nccl_lib, addr_to_name):
    ...
    """nm $TORCH_CUDA_LIB | grep ncclKernel_ | grep -v Broadcast | c++filt"""

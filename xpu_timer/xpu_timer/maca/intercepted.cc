...
InterceptManager::handleNccl(
  ...
  auto fn = [sym, nccl_info, grid_dim,
             block_dim](MacaGpuTimer* timer) -> MacaGpuTimer::FnReturn {
#else
  auto fn = [sym,
             nccl_info](MacaGpuTimer* timer) -> MacaGpuTimer::FnReturn {
#endif
    timer->trace->Clear();
...
InterceptManager::handleFa(void** args, const InterceptSymbol* sym) {
  ...
  auto fn = [sym,
             fa_params](MacaGpuTimer* timer) -> MacaGpuTimer::FnReturn {
    timer->trace->Clear();
...
InterceptManager::handleCudaLaunchKernel(const void* func, dim3 gridDim,
  ...
  else if (sym->func_type == "NCCL")
  {
    //TODO
    *skip = false;
    return cuda_launch_kernel_nccl_;
  }
...
InterceptManager::deviceMemory(const std::string& name, const size_t size,
  ...
  return [name, size, kind, is_host](MacaGpuTimer* timer) {
    timer->trace->Clear();
    timer->trace->set_kernel_type(
...
std::function<MacaGpuTimer::FnReturn(MacaGpuTimer*)>
    InterceptManager::cuda_launch_kernel_exc_default_ = [](MacaGpuTimer*) {
      return std::make_tuple("CudaLaunchKernelExC_UNKNOWN", 0,
                             xpu_timer::Labels{});
    };
std::function<MacaGpuTimer::FnReturn(MacaGpuTimer*)>
    InterceptManager::cuda_launch_kernel_default_ = [](MacaGpuTimer*) {
      return std::make_tuple("CudaLaunchKernel_UNKNOWN", 0,
                             xpu_timer::Labels{});
    };
std::function<MacaGpuTimer::FnReturn(MacaGpuTimer*)>
    InterceptManager::cuda_launch_kernel_nccl_ = [](MacaGpuTimer*) {
      return std::make_tuple("CudaLaunchKernel_NCCL", 1,
                             xpu_timer::Labels{});
    };

...
class MacaGpuTimer : public XpuTimer {
 ...
 public:
  ...
  explicit MacaGpuTimer() {
    // cudaEventCreate(&start_event_);
    cudaEventCreateWithFlags(&stop_event_, cudaEventDisableTiming);
    hang_counter_ = 0;
    trace = new hook::KernelTrace();  // MacaGpuTimer is pooling, trace should
                                      // never free.
  }
  ...
  void reset(std::function<MacaGpuTimer::FnReturn(MacaGpuTimer*)> cb,
             const std::string_view& type);
  // kernel trace object
  ...
  void reset_cb(std::function<MacaGpuTimer::FnReturn(MacaGpuTimer*)> cb);
 private:
  ...
  class MatmulBuilderCallback {
   public:
    ...
    MacaGpuTimer::FnReturn operator()(MacaGpuTimer*);
...
class InterceptManager {
 ...
 private:
  ...
  std::function<MacaGpuTimer::FnReturn(MacaGpuTimer*)> handleFa(
      void** args, const InterceptSymbol* sym);
  std::function<MacaGpuTimer::FnReturn(MacaGpuTimer*)> handleNccl(
  ...
  static std::function<MacaGpuTimer::FnReturn(MacaGpuTimer*)>
      cuda_launch_kernel_exc_default_;
  static std::function<MacaGpuTimer::FnReturn(MacaGpuTimer*)>
      cuda_launch_kernel_default_;
  static std::function<MacaGpuTimer::FnReturn(MacaGpuTimer*)>
      cuda_launch_kernel_nccl_;
 ...
 public:
...
#if defined(CUDA_LAUNCH_EXC)
  std::function<MacaGpuTimer::FnReturn(MacaGpuTimer*)>
  ...
  std::function<MacaGpuTimer::FnReturn(MacaGpuTimer*)> deviceMemory(

...
def save_checkpoint(
    ...
    if coordinator.is_master():
        print("booster.save_model start")
    booster.save_model(model, os.path.join(save_dir, "modeling"), shard=True)

    if coordinator.is_master():
        print("booster.save_optimizer start")
    booster.save_optimizer(optimizer, os.path.join(save_dir, "optimizer"), shard=True)

    if coordinator.is_master():
        print("booster.save_lr_scheduler start")
    booster.save_lr_scheduler(lr_scheduler, os.path.join(save_dir, "lr_scheduler"))
    if coordinator.is_master():
        print("booster.save_lr_scheduler end")
    running_states = {
        "epoch": epoch,
        "step": step,
    ...
    if coordinator.is_master():
        ...
        with open(os.path.join(root_dir, "latest.txt"), 'w') as fp:
            fp.write(str(step))
...
def load_checkpoint(
    ...
    """
    with open(os.path.join(load_dir, "latest.txt"), 'r') as fp:
        step = fp.read()
